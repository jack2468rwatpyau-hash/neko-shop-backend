...
app.use(helmet());
app.use(cors({
  origin: '*', // အားလုံးကို ခွင့်ပြုရန်
  methods: ['GET', 'POST', 'PUT', 'DELETE']
}));
app.use(bodyParser.json({ limit: '10mb' })); // Base64 ပုံကြီးတွေ လက်ခံရန်

// JSON Data File Path
const DATA_FILE = path.join(__dirname, 'data.json');

// Helper: Read Data (Ensure file exists)
const readData = () => {
  if (!fs.existsSync(DATA_FILE)) {
    // If file doesn't exist, create it with default structure
    fs.writeFileSync(DATA_FILE, JSON.stringify({ products: [], orders: [], users: [], settings: {} }));
  }
