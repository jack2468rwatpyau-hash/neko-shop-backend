...
//   1) Global: once 10 finished orders pile up across the whole shop,
//      archive the oldest 10 to Telegram and delete them.
//   2) Per-customer: once a single logged-in customer has more than 7
//      finished orders in their own history, archive+delete the extras
//      so "My Orders" doesn't grow forever either.
...
async function archiveOrdersIfNeeded() {
  try {
    ...
    if (archivable.length >= ARCHIVE_BATCH_SIZE) {
      // Global rule takes priority for this pass.
