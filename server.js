...
// ---------- Telegram Notification (စာသား+ပုံ၊ အချိန်အတိအကျ) ----------
async function sendTelegramOrderNotification(order) {
  ...
  const now = new Date();
  const orderTime = now.toLocaleString('en-GB', { hour12: false }); // 2026-07-24 14:30:00
...
// ---------- Auto Archive (အော်ဒါဟောင်း ၇ ခုပြည့်ရင် ဖျက်မယ်) ----------
const ARCHIVE_BATCH_SIZE = 7;
const ARCHIVABLE_STATUSES = ['approved', 'shipped', 'cancelled'];

async function archiveOldOrdersIfNeeded() {
  try {
    const data = readData();
    const archivable = data.orders.filter((o) => ARCHIVABLE_STATUSES.includes(o.status));
    if (archivable.length < ARCHIVE_BATCH_SIZE) return;

    const batch = [...archivable]
      .sort((a, b) => new Date(a.created_at) - new Date(b.created_at))
      .slice(0, ARCHIVE_BATCH_SIZE);

    const batchIds = new Set(batch.map((o) => o.order_id));
    const fresh = readData();
    fresh.orders = fresh.orders.filter((o) => !batchIds.has(o.order_id));
...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    sendTelegramOrderNotification(order); // Image & Time sent here
...
app.put('/api/admin/orders/:orderId', adminAuth, async (req, res) => {
  try {
    ...
    order.status = status;
    await writeData(data);
    archiveOldOrdersIfNeeded(); // Auto delete old orders when 7 are full
    res.json({ order });
  } catch (err) {
    console.error(err);
...
app.post('/api/admin/products', adminAuth, async (req, res) => {
  try {
    ...
    const product = {
      ...
      images: images || {}, // Size အလိုက် ပုံတွေ
      desc: desc || '',
      sizes: Array.isArray(sizes) ? sizes : [],
      colors: Array.isArray(colors) ? colors : [],
      stock: Number(stock) || 0,
      payment_type: payment_type || 'cod',
      estimated_delivery: estimated_delivery || '3-5 business days'
    };
...
app.put('/api/admin/products/:id', adminAuth, async (req, res) => {
  try {
    ...
    const fields = ['name', 'price', 'oldPrice', 'category', 'image', 'images', 'desc', 'sizes', 'colors', 'stock', 'payment_type', 'estimated_delivery'];
    fields.forEach(f => {
      if (req.body[f] !== undefined) {
        if (f === 'sizes' || f === 'colors') product[f] = Array.isArray(req.body[f]) ? req.body[f] : [];
        else if (f === 'price' || f === 'oldPrice' || f === 'stock') product[f] = Number(req.body[f]) || 0;
