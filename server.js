...
if (!JWT_SECRET) {
  console.error('FATAL: JWT_SECRET is not set. Add it to your .env file / Railway Variables.');
  process.exit(1);
}
if (!process.env.ADMIN_USERNAME || !process.env.ADMIN_PASSWORD) {
  console.error(
    'FATAL: ADMIN_USERNAME and/or ADMIN_PASSWORD is not set. Add both to your .env file / Railway Variables — the admin-auth middleware would crash the whole server on startup without them.'
  );
  process.exit(1);
}
...
// Quick way to check from a browser that the server process is alive at all,
// separate from any /api/* route (helps tell "server down" apart from "route error").
app.get('/', (req, res) => res.json({ status: 'ok', service: 'neko-shop-backend' }));
...
app.post('/api/admin/products', adminAuth, async (req, res) => {
  try {
    const { name, price, oldPrice, categories, image, gallery, video, desc, sizes, colors, stock, payment_type, estimated_delivery, variant_prices } = req.body;
