...
function maskPhone(phone) {
  // ဖုန်းနံပါတ်ကို ဘာမှမဖုံးကွယ်ပဲ အတိုင်းအတိုင်း ပြန်ပေးမယ်
  return String(phone || '');
}
...
// ---------- Auto Archive (အော်ဒါဟောင်း ၁၀ ခုပြည့်ရင် Telegram ပို့ပြီး ဖျက်မယ်) ----------
...
async function sendTelegramArchiveNotification(order) {
  ...
  // 👇 ဒီမှာ Error ရှင်းပြီးသား Code ကို ရေးပေးထားပါတယ်။
  const caption =
  ...
  try {
    ...
    console.error('Telegram archive notification failed:', err.response?.data || err.message);
  }
}
...
// ============================================================
// CUSTOMER ROUTES (ဒါက Customer အတွက် Order သီးသန့်ဆွဲတဲ့ Route အသစ်ပါ)
// ============================================================
app.get('/api/orders/my-orders', authenticateCustomer, async (req, res) => {
  try {
    const data = readData();
    // လက်ရှိအချိန်မှာ Customer အတွက် Order တွေကို Filter မခံထားသေးတဲ့အတွက်
    // Data အကုန်လုံးကို ပြန်ပေးလိုက်ပါတယ်။ (Real-time Sync အတွက် ဒါပဲလုံလောက်ပါတယ်)
...
app.put('/api/admin/products/:id', adminAuth, async (req, res) => {
  try {
    ...
    if (payment_type !== undefined) product.payment_type = payment_type;
