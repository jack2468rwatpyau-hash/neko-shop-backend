...
// ✅ လုံခြုံရေး CORS (Dynamic: Local + Vercel အတွက်)
...
function readData() {
  ...
  // Migrate old single-category products to the new multi-category array.
...
// ---------- Direct chat between a customer and admin ----------
  ...
  try {
    ...
    if (token && chatId) {
      ...
      axios
        ...
        .catch((err) => console.error('Telegram chat notify failed:', err.response?.data || err.message));
    }
...
app.get('/api/admin/messages/:userId', adminAuth, (req, res) => {
  ...
  const thread = data.messages
    ...
    .sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
  // Mark customer messages as read by admin now that they've opened the thread.
  let changed = false;
  data.messages.forEach((m) => {
    if (m.user_id === userId && m.sender === 'customer' && !m.read_by_admin) {
...
// Admin-set password — passwords are one-way hashed (bcrypt) and can never be
// read back in plain text, so this lets admin issue a new one for support
// purposes instead (e.g. to relay to the customer via chat).
...
app.get('/api/admin/users', adminAuth, (req, res) => {
  ...
  const users = data.users
    .map(({ password, ...safeUser }) => ({
      ...safeUser,
      order_count: data.orders.filter((o) => o.user_id === safeUser.id).length,
    }))
    .sort((a, b) => (b.customer_number || 0) - (a.customer_number || 0));
  res.json(users);
});
...
app.post('/api/admin/products', adminAuth, async (req, res) => {
  try {
    const { name, price, oldPrice, category, categories, image, images, video, desc, sizes, colors, stock, payment_type, estimated_delivery, variant_prices } = req.body;
    ...
    const product = {
      ...
      name,
      // Base/"from" price — if per-variant prices are set, show the cheapest variant on the card.
      price: variantPriceValues.length ? Math.min(...variantPriceValues) : Number(price),
      oldPrice: oldPrice ? Number(oldPrice) : null,
      categories: finalCategories,
      category: finalCategories[0], // kept for backward compatibility with older clients
...
// Chat messages older than 30 days are auto-deleted. This is age-based
// (unlike order archiving, which triggers off admin actions) so it runs on
// a timer: once at startup, then once a day.
...
async function cleanupOldMessages() {
  try {
    ...
    if (data.messages.length !== before) {
      await writeData(data);
      console.log(`Cleaned up ${before - data.messages.length} chat message(s) older than ${MESSAGE_RETENTION_DAYS} days.`);
    }
  } catch (err) {
    console.error('Message cleanup failed:', err);
  }
}
cleanupOldMessages();
