...
app.get(
  ...
  passport.authenticate('google', { session: false, failureRedirect: `${process.env.FRONTEND_URL || ''}/auth-failed.html` }),
  (req, res) => {
    const user = req.user;
    const token = jwt.sign({ id: user._id.toString(), email: user.email, role: user.role }, JWT_SECRET, { expiresIn: '7d' });
    const needsPhone = !user.phone;
    const base = process.env.FRONTEND_URL || '';
    res.redirect(`${base}/auth-success.html?token=${token}&needsPhone=${needsPhone}`);
  }
);

// Called from the "add your name & phone" popup after a first-time Google login.
