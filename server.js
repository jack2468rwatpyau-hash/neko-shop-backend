...
app.post('/api/admin/products', adminAuth, async (req, res) => {
  try {
    const { name, price, oldPrice, category, categories, image, images, gallery, variant_images, video, desc, sizes, colors, stock, payment_type, estimated_delivery, variant_prices } = req.body;
    ...
    const finalGallery = Array.isArray(gallery) ? gallery.slice(0, 8) : [];
    ...
    const product = {
      ...
      image: finalGallery[0] || image || 'https://picsum.photos/400/400',
...
app.put('/api/admin/products/:id', adminAuth, async (req, res) => {
  try {
    ...
    if (gallery !== undefined) {
      product.gallery = Array.isArray(gallery) ? gallery.slice(0, 8) : [];
