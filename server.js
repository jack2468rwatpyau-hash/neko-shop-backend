...
async function sendTelegramOrderCard(order, { title, footer }) {
  ...
  const itemLines = order.items
    .map((item) => {
      const tag = item.product_number ? `#${item.product_number}${item.variant_code ? `-${item.variant_code}` : ''} ` : '';
...
function pushNotification(data, userId, orderId, status) {
  ...
  // Keep at most the most recent 20 notifications per customer.
...
app.post('/api/admin/products', adminAuth, async (req, res) => {
  try {
    ...
    const product = {
      ...
      product_number: nextProductNumber(data.products),
      name,
      price: Number(price),
